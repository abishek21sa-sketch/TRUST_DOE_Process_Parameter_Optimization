...
def _case_trust():
    ...
    gp_rmse=float(np.sqrt(np.mean(np.square(gp_err))))
    quadratic_rmse=float(np.sqrt(np.mean(np.square(quad_err))))
    comparisons=[
        {'model':'rbf_gaussian_process','task':'uniformity regression','validation':'leave-one-out','rmse':gp_rmse,'selected':gp_rmse<=quadratic_rmse},
        {'model':'quadratic_response_surface','task':'uniformity regression','validation':'leave-one-out','rmse':quadratic_rmse,'selected':quadratic_rmse<gp_rmse},
    ]
    selected=min(comparisons,key=lambda row:row['rmse'])['model']
    return {'name':CFG['case'],'status':'ACTIVE','profile':prof,'observed_runs':len(r),'gp_leave_one_out_rmse':gp_rmse,'quadratic_leave_one_out_rmse':quadratic_rmse,'model_comparison':comparisons,'selected_model':selected,'selection_metric':'lowest leave-one-out RMSE',
